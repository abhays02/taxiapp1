...
use App\Http\Controllers\Resource\TripResource;
use App\PeakHour;
...
class UserApiController extends Controller
{
    ...
    public function login(Request $request)
    {
        try {
            ...
            $results = User::where('email', $email)->orWhere('mobile', $email)->first();

            // if (empty($results)) {
            //     return response()->json(['message' => 'User Don\'t exist', 'is_available' => false],422);
            // }
            $request->username = $results->email;
        } catch (Exception $e) {
            return response()->json(['error' => trans('api.something_went_wrong')], 500);
        }

        //$request->username = User::where('email', $request->username)->get();
    ...
    public function send_request(Request $request)
    {
        ...
        try {
            if ($request->service_required == 'rental') {
                $details = 'https://maps.googleapis.com/maps/api/distancematrix/json?origins=' . $request->s_latitude . ',' . $request->s_longitude . '&destinations=' . $request->s_latitude . ',' . $request->s_longitude . '&mode=driving&sensor=false&key=' . config('constants.map_key');
            } else {
                $details = 'https://maps.googleapis.com/maps/api/distancematrix/json?origins=' . $request->s_latitude . ',' . $request->s_longitude . '&destinations=' . $request->d_latitude . ',' . $request->d_longitude . '&mode=driving&sensor=false&key=' . config('constants.map_key');
            }
...
// dd($package_time_all);
            ...
            return response()->json(['data' => $UserRequests,
                ...
                'waitingStatus' => (new TripController())->waiting_status($UserRequests->request_id),
                'waitingTime' => (new TripController())->total_waiting($UserRequests->request_id),
    ...
    public function estimated_fare(Request $request)
    {
        ...
        //try {
            ...
            ->get();
                ...
                if ($Providers->count() <= config('constants.surge_trigger',0) && $Providers->count() > 0) {
...
//                }
    ...
    public function addMoneyRaz(Request $request)
    {
        ...
        $wallet               = (new TripController())->userCreditDebit($request->amount, $User->id, 1);
        (new SendPushNotification())->WalletMoney($User->id, currency($request->amount));

        return response()->json(['success' => currency($request->amount) . ' ' . trans('api.added_to_your_wallet'),
            'message'                      => currency($request->amount) . ' ' . trans('api.added_to_your_wallet'),
            'wallet_balance'               => $User->wallet_balance, 'user' => $User, ]);
    }
}
