...
class AdminController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        try {
            ...
            //  map data
            $sections=UserRequests::whereDate('created_at', '>=', Carbon::now()->subDays(60));
            $sections=$sections->get();
            $newarray=[];
            foreach ($sections as $row) {
                $section['latLng']=[$row->d_latitude, $row->d_longitude];
                $section['name']  =optional(optional($row)->user)->first_name . ' ' . optional(optional($row)->user)->last_name;
                array_push($newarray, $section);
            }
            ...
            $totalScheduledGraph         =[];
            $totalCancelledGraph         =[];
            $PassengerstotalcountGraph   =[];
            $user_cancelledgraph         =[];
            $totalprovider_cancelledgraph=[];
            $totalprovidergraph          =[];

            $totalrevenuegraph=[];
            while (strtotime($date) <= strtotime($end_date)) {
                $date  = date('Y-m-d', strtotime('+1 day', strtotime($date)));
                $Users = UserRequests::whereDate('created_at', $date)->get();
                $providergraph = Provider::whereDate('created_at', $date)->get()->count();

                array_push($totalprovidergraph, $providergraph);

                $provider_cancelledgraph=$Users->where('status', 'CANCELLED')->where('cancelled_by', 'PROVIDER')->count();
                array_push($totalprovider_cancelledgraph, $provider_cancelledgraph);
                $usercancelledgraph=$Users->where('status', 'CANCELLED')->where('cancelled_by', 'USER')->count();
                array_push($user_cancelledgraph, $usercancelledgraph);
                $countCompleted=$Users->where('status', 'COMPLETED')->count();
                $countCancelled=$Users->where('status', 'CANCELLED')->count();
                array_push($userOpen, $countCompleted);
                array_push($userClose, $countCompleted);
                $counttotalRides=$Users->count();
                array_push($totalRidesgraph, $counttotalRides);
                $countSCHEDULED=$Users->where('status', 'SCHEDULED')->count();
                array_push($totalScheduledGraph, $countSCHEDULED);
                $countCancelledGraph=$Users->where('status', 'CANCELLED')->count();
                array_push($totalCancelledGraph, $countCancelledGraph);
                $Passengerstotalcount = User::whereDate('created_at', $date);
                $Passengerstotalcount=$Passengerstotalcount->get()->count();
                array_push($PassengerstotalcountGraph, $Passengerstotalcount);
                $revenuegraph = UserRequestPayment::whereDate('created_at', $date)->sum('total');
                array_push($totalrevenuegraph, $revenuegraph);
            }
            $jsondata['PassengerstotalcountGraphData']=json_encode($PassengerstotalcountGraph);
            $jsondata['totalCancelledGraph']          =json_encode($totalCancelledGraph);

            $jsondata['totalRidesGraph']=json_encode($totalRidesgraph);
            $jsondata['ScheduledGraph'] =json_encode($totalScheduledGraph);
            $jsondata['open']           = json_encode($userOpen);
            $jsondata['close']          = json_encode($userClose);

            $jsondata['user_cancelledgraph']=json_encode($user_cancelledgraph);

            $jsondata['totalprovider_cancelledgraph']=json_encode($totalprovider_cancelledgraph);
            $jsondata['totalprovidergraph']          =json_encode($totalprovidergraph);

            $jsondata['totalrevenuegraph']=json_encode($totalrevenuegraph);
            //dd($jsondata['totalrevenuegraph']);
            // map data end
            ...
            //     }
            // } else {
            $rides          = UserRequests::has('user')->limit(10)->orderBy('id', 'desc')->get();
            $users    = User::get()->count();
            $provider = Provider::get()->count();
            ...
            $totalRides         = UserRequests::get()->count();
            //}
            ...
            //dd($geoFencing);
            //admin.dashboard
            return view('admin.dashboard_1', compact('jsondata', 'mapdata', 'providers', 'fleet', 'provider', 'scheduled_rides', 'users', 'rides', 'totalRides', 'user_cancelled', 'provider_cancelled', 'cancel_rides', 'revenue', 'wallet', 'geoFencing'));
        } catch (Exception $e) {
            return redirect()->route('admin.user.index')->with('flash_error', 'Something went wrong with the Panel!');
        }
    }
    ...
    public function settings_store(Request $request)
    {
        ...
        if (!file_exists($config)) {
            ...
            $data = "<?php 
				...
				return array(
					...
					'site_email_logo' => '',
					'site_icon' => '',
					'site_copyright' => '© 2021 Thinkin cab',
					...
					'peak_percentage' => '0',
                    'surge_trigger' => '0',
                    'outstation_base_km' => '0',
                    'limit_message' => '',
					'demo_mode' => '0',
					'booking_prefix' => 'MTBX',
					'sos_number' => '911',
					'contact_number' => '',
					'contact_email' => 'admin@thinkindragon.com',
					'environment' => '',
					'ios_push_password' => '',
					'android_push_key' => '',
        ...
        if (!$request->has('rental')) {
            $search_text    = "'rental' => '1'";
            $value_text     = "'rental' => '0'";
            $change_content = str_replace($search_text, $value_text, $change_content);
        }
        if (!$request->has('outstation')) {
            $search_text    = "'outstation' => '1'";
            $value_text     = "'outstation' => '0'";
            $change_content = str_replace($search_text, $value_text, $change_content);
        }
        ...
        if (!$request->has('manual_request')) {
            ...
            $change_content = str_replace($search_text, $value_text, $change_content);
                ...
                if (!$request->has('track_distance')) {
            $search_text    = "'track_distance' => '1'";
            $value_text     = "'track_distance' => '0'";
            $change_content = str_replace($search_text, $value_text, $change_content);
        }
        ...
        try {
            if ((isset($request->provider_id) && $request->provider_id != null) ||
                ...
                if (isset($request->provider_id) && $request->provider_id != null) {
                    ...
                    $rides = UserRequests::get()->orderBy('id', 'desc');

                    $cancel_rides = UserRequests::get()->where('status', 'CANCELLED');
    ...
    public function statement_fleet()
    {
        try {
            ...
            foreach ($Fleets as $index => $Fleet) {
                $Rides = UserRequests::get()->pluck('id');
    ...
    public function poly_check_request_city($id)
    {
        $range_data = GeoFencing::where('id', $id)->get();
        //dd($range_data);
