...
class Helper
{
    ...
    {
        //\Log::alert('OTP hit by : '.$mobile);
        $user_id   = '11787';
        $apikey    = '';
        $shortcode = 'NotifyDEMO';

        $message = 'Your ' . config('constants.site_title') . ' OTP is : ' . $otp;

        //$finalURL = '';//'https://app.notify.lk/api/v1/send?user_id=' . $user_id . '&api_key=' . $apikey . '&sender_id=' . $shortcode . '&to=' . urlencode('94' . $mobile) . '&message=' . urlencode($message) ;
        //\Log::alert('OTP hit responce : '.$finalURL);

        $finalURL = '';//'https://api.msg91.com/api/sendotp.php?mobile=91' . $mobile . '&authkey=' . $apikey . '&route=4&sender=zedcab&otp=' . $otp;
