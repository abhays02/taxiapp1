    ...
    public function calculateFare($request, $cflag=0)
    {
        ...
        $geo_fencing=$this->poly_check_new((round($request->s_latitude, 6)), (round($request->s_longitude, 6)));
            ...
            if (!empty($location['errors'])) {
                ...
                $requestarr['geo_fencing'] =$geo_fencing;
...
public function poly_check_new($s_latitude, $s_longitude)
    ...
    public function applyPriceLogic($requestarr, $iflag=0)
    {
        ...
        )->whereid($requestarr['geo_fencing'])->first();
        ...
        \Log::alert("geofence service");
        \Log::alert($geo_fencing_service_type);
        //$rental = ceil($requestarr['rental_hours']);
        if ($geo_fencing_service_type != null) {
            $per_minute       =$geo_fencing_service_type->service_geo_fencing->minute; //PM
            $per_hour         =$geo_fencing_service_type->service_geo_fencing->hour; //PH
            $per_kilometer    =$geo_fencing_service_type->service_geo_fencing->price; //PKM
            $base_distance    =$geo_fencing_service_type->service_geo_fencing->distance; //BD
            $base_price       =$geo_fencing_service_type->service_geo_fencing->fixed ? $geo_fencing_service_type->service_geo_fencing->fixed : 0; //BP
            //TODO ALLAN - Tarifa mínima
            $min_price=$service_type->min_price; //BP
            if ($iflag == 1)
                \Log::alert("Geo price hit ".$per_minute." ".$per_hour." ".$per_kilometer." ".$base_distance." ".$base_price." ".$total_kilometer." last is total Km");
        } else {
