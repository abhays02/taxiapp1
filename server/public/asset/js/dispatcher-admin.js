  ...
  handleAutoRequest(trip) {
    if (trip.status == "SEARCHING" && trip.current_provider_id != 0) {
      ...
      $(".container-fluid")
        .first()
        .before(
          '<div class="alert alert-danger notification"><button type="button" class="close" data-dismiss="alert">×</button><p style="margin-top:10px;">Ride is auto assigned. You cannot manually assign drivers.</p></div>'
        );
      setTimeout(function () {
        $(".notification").fadeOut("fast", "linear", function () {
          $(".notification").delay(5000).remove();
        });
      }, 5000);
    }
  }
      ...
      <nav className="navbar navbar-expand-lg">
        ...
        <div className="navbar-toggleable-sm" id="process-filters">
          <ul className="nav navbar-nav dispatcher-nav">
            ...
            <li
              className={this.isActive("warning")}
              onClick={this.filter.bind(this, "warning")}
            >
              <span className="nav-link" href="#">
                Warning
              </span>
...
class DispatcherList extends React.Component {
  ...
  componentDidMount() {
    ...
    window.TripTimer = setInterval(() => this.getTripsUpdate(), 1000);
  }
  ...
  getTripsUpdate() {
    $.get(
      "/admin/dispatcher/trips?type=SEARCHING",
      function (result) {
...
class DispatcherCancelledList extends React.Component {
  ...
  getTripsUpdate() {
    $.get(
      "/admin/dispatcher/trips?type=CANCELLED",
      function (result) {
        if (result.hasOwnProperty("data")) {
          this.setState({
            data: result,
      ...
      return (
        <div className="il-item " key={trip.id}>
          <a className="text-black" href="#">
            <div className="card card-inverse bg-warning">
              <div className="card-body">
                <p className="mb-0-5 card-title">
                  {trip.user.first_name} {trip.user.last_name}
                  {trip.status == "COMPLETED" ? (
                    <span className="badge badge-round badge-success pull-right">
                      {" "}
                      {trip.status}{" "}
                    </span>
                  ) : trip.status == "CANCELLED" ? (
                    <span className="badge badge-round badge-danger pull-right">
                      {" "}
                      {trip.status}{" "}
                    </span>
                  ) : trip.status == "SEARCHING" ? (
                    <span className="badge badge-round badge-warning pull-right">
                      {" "}
                      {trip.status}{" "}
                    </span>
                  ) : trip.status == "SCHEDULED" ? (
                    <span className="badge badge-round badge-primary pull-right">
                      {" "}
                      {trip.status}{" "}
                    </span>
                  ) : (
                    <span className="badge badge-round badge-info pull-right">
                      {" "}
                      {trip.status}{" "}
                    </span>
                  )}
                </p>
                <h6 className="card-heading">From: {trip.s_address}</h6>
                <h6 className="card-heading">
                  To: {trip.d_address ? trip.d_address : "Not Selected"}
                </h6>
                <h6 className="card-heading">Payment: {trip.payment_mode}</h6>
                <progress
                  className="progress-bar progress-bar-striped progress-bar-sm"
                  max="100"
                ></progress> 
...
class DispatcherRequest extends React.Component {
  ...
  componentDidMount() {
    ...
    $('#schedule_time_1').datetimepicker({ footer: true, modal: true });
    // $("#schedule_time").datetimepicker({
    //   minDate: window.Thinkin.minDate,
    //   maxDate: window.Thinkin.maxDate,
    // });
    jQuery("#schedule_time").datetimepicker({ footer: true, modal: true });
            ...
            <div className="row">
              <div className="col-xs-6">
                <div className="form-group">
                  <label className="bmd-label-floating" htmlFor="first_name">
                    First Name
                  </label>
                  <input
                    type="text"
              ...
              <div className="col-xs-6">
                <div className="form-group">
                  <label className="bmd-label-floating" htmlFor="last_name">
                    Last Name
                  </label>
                  <input
                    type="text"
              ...
              <div className="col-xs-6">
                <div className="form-group">
                  <label className="bmd-label-floating" htmlFor="mobile">
                    Phone
                  </label>
                  <input
                    type="text"
              ...
              <div className="col-xs-12">
                <div className="form-group">
                  <label className="bmd-label-floating" htmlFor="s_address">
                    Pickup Address
                  </label>
                ...
                <div className="form-group">
                  <label className="bmd-label-floating" htmlFor="d_address">
                    Dropoff Address
                  </label>
                ...
                <div className="form-group">
                  <label className="bmd-label-floating" htmlFor="schedule_time">
                    Schedule Time
                  </label>
                  <input
                    type="text"
                ...
                <div className="form-group">
                  <label htmlFor="estimated" className="estimate_amount">
                    Estimated Amount : <span id="estimated">0</span>
                  </label>
                </div>
                <div className="form-group">
          ...
          <a className="text-black" href="#">
            <div className="card card-inverse bg-info">
              <div className="card-body">
                ...
                <h6 className="card-heading">Rating: {provider.rating}</h6>
                <h6 className="card-heading">Phone: {provider.mobile}</h6>
                <h6 className="card-heading">
                  Type: {provider.service.service_type.name}
                </h6>
              </div>
            </div>
          </a>
