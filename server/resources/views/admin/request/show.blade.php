...
<div class="page">
    <div class="page-header">
        ...
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">@lang('admin.request.request_details')</li>
        </ol>
        <div class="page-header-actions">
            ...
            <button type="button" class="btn btn-sm btn-icon btn-default btn-outline btn-round" data-toggle="tooltip"
                ...
                <a href="{{ URL::previous() }}"> <i class="fa fa-angle-left"
                        aria-hidden="true"></i>@lang('admin.back')</a>
            </button>
                ...
                <div class="content-area py-1">
                    <div class="container-fluid">
                        <div class="box box-block bg-white">
                            <h4 class="example-title">@lang('admin.request.request_details')</h4>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <dl class="row">
                                        ...
                                        <dd class="col-sm-8">
                                            {{ $request->distance ? $request->distance : '0' }}{{$request->unit}}</dd>

                                        @if($request->status == 'SCHEDULED')
                                        <dt class="col-sm-4">@lang('admin.request.ride_scheduled_time')</dt>
                                        <dd class="col-sm-8">
                                            @if($request->schedule_at != "")
                                            {{ date('Y-m-d H:i:s', strtotime($request->schedule_at)) }}
                                        ...
                                        <dd class="col-sm-8">
                                            ...
                                            {{ date('Y-m-d H:i:s', strtotime($request->started_at)) }}
                                            @else
                                            -
                                            @endif
                                        </dd>
                                        ...
                                        <dd class="col-sm-8">
                                            ...
                                            {{ date('Y-m-d H:i:s', strtotime($request->finished_at)) }}
                                            @else
                                            -
                                            @endif
                                        </dd>
                                        ...
                                        @if($request->service_required =='none')
                                            <dd class="col-sm-8">Daily Ride</dd>
                                        @elseif($request->service_required =='rental')
                                            <dd class="col-sm-8">Rental</dd>
                                        @elseif($request->service_required =='outstation')
                                            <dd class="col-sm-8">Out Station</dd>
                                        @endif
                                        @if($request->payment)
                                        @if($request->service_required =='none')
                                        ...
                                        <dd class="col-sm-8">{{ currency($request->payment->fixed) }}</dd>
                                            ...
                                            @endif
                                        @elseif($request->service_required =='rental')
                                        <dt class="col-sm-4">Rental package</dt>
                                            @foreach ($request->service_type->rental_hour_package as $item)
                                                @if($item->id == $request->rental_hours)
                                                    <dd class="col-sm-8">{{$item->km}} Km /{{$item->hour}} Hr </dd>
                                                @endif
                                            @endforeach
                                        
                                        @elseif($request->service_required =='outstation')
                                        <dt class="col-sm-4">Leave on</dt>
                                        <dd class="col-sm-8">{{$request->out_leave}}</dd>
                                        <dt class="col-sm-4">Return on</dt>
                                        <dd class="col-sm-8">{{$request->out_return}}</dd>
                                        @endif
                                        ...
                                        <dt class="col-sm-4" style="padding-right:0px;">
                                            @lang('admin.request.waiting_commission')</dt>
                        ...
                        <dd class="col-sm-8">{{ currency($request->payment->surge) }}</dd> -->
                                        ...
                                        <dd class="col-sm-8">
                                            {{ currency($request->payment->total+$request->payment->tips) }}</dd>
                        ...
                        <dd class="col-sm-8">{{ currency($request->payment->payable) }}</dd> -->
                                        ...
                                        <dd class="col-sm-8">
                                            {{ currency($request->payment->provider_pay - $request->payment->discount) }}
                                        </dd>

                                        <dt class="col-sm-4">@lang('admin.request.admin_commission')</dt>
...
<script
    src="https://maps.googleapis.com/maps/api/js?key={{ Config::get('constants.map_key') }}&libraries=places&callback=initMap"
    async defer></script>
@endsection